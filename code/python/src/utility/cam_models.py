...
def stitch_rgb_image(image_data_list, image_param_list, fisheye_model, subimage_fov=60, fisheye_fov=180):
    ...
    fisheye_image = np.zeros((fisheye_image_height, fisheye_image_width, channel_number), np.float32)
    fisheye_image_weight = np.zeros((fisheye_image_height, fisheye_image_width), np.float32)
    for index in range(0, len(image_data_list)):
        fisheye_image_weight_subimg = np.zeros((fisheye_image_height, fisheye_image_width), np.float32)
        fisheye_image_subimage = np.zeros((fisheye_image_height, fisheye_image_width, channel_number), np.float32)
        ...
        available_pixels_weight = np.ones(available_pixels_list.shape,  np.float32)
...
def sample_rgb_image(img, model, fov=[60, 60], canvas_size=[400, 400], sample_grid_size=[3, 3]):
    ...
    for index in range(0, len(sub_image_param_list)):
        ...
        tangential_img = np.zeros(tuple(canvas_size) + (channel_number,), dtype=np.float32)
...
def sample_img(img, cam_model, fov=53, run_midas=False):
    ...
    equirect_3D_points_rgb = np.zeros((7, equirect_3D_points.shape[-1]), dtype=np.float32)
    ...
    fisheye2equirec = np.zeros((3, equirect_3D_points.shape[-1]), dtype=np.float32)
...
def generate_camera_orientation(hfov_fisheye, vfov_fisheye, hfov_pinhole, vfov_pinhole, horizontal_size, vertical_size, padding=0):
    ...
    z_rotation = np.zeros(x_rotation.shape, np.float32)
